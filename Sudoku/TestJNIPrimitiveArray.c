...
JNIEXPORT jintArray JNICALL Java_TestJNIPrimitiveArray_sumAndAverage
   ...
   if (solveSudoku(grid, 0, 0)==1){
      ...
      for(int i = 0; i<N;i++){
         for(int j = 0; j<N; j++){
            gridv2[i * N + j] = grid[i][j];
         }
      }
      
      jintArray outJNIArray = (*env)->NewIntArray(env, 81);  // allocate
      if (NULL == outJNIArray) return NULL;
      (*env)->SetIntArrayRegion(env, outJNIArray, 0 , 81, gridv2);  // copy
      return outJNIArray;
